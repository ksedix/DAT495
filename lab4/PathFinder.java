...
public class PathFinder<Node> {
    ...
    public Result searchUCS(Node start, Node goal) {
         ...
         *************************************************************************************************/
        pqueue.add(new PQEntry(start,0,null,null,0));
        while (pqueue.size()>0) {
            //use .equals() or ==
            ...
            if (!visited.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : graph.outgoingEdges(entry.node)) {
                    pqueue.add(new PQEntry(edge.to(), entry.costToHere + edge.weight(), edge, entry,0));
                }
    ...
    public Result searchAstar(Node start, Node goal) {
        ...
        Queue<PQEntry> pqueue = new PriorityQueue<>(Comparator.comparingDouble(e -> e.costToGoal));
        Set<Node> visited = new HashSet<>();
        /*************************************************************************************************
         * TODO: Task 1a+c                                                                               *
         * Change here.                                                                                  *
         * Note: Every time you remove a node from the priority queue, you should increment `iterations` *
         *************************************************************************************************/
        pqueue.add(new PQEntry(start,0,null,null, graph.guessCost(start,goal)));
        while (pqueue.size()>0) {
            //use .equals() or ==
            PQEntry entry = pqueue.remove();
            iterations++;
            if (!visited.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : graph.outgoingEdges(entry.node)) {
                    Double costToGoal = entry.costToHere + edge.weight() + graph.guessCost(edge.to(),goal);
                    pqueue.add(new PQEntry(edge.to(), entry.costToHere + edge.weight(), edge, entry, costToGoal));
                }
    ...
    private List<DirectedEdge<Node>> extractPath(PQEntry entry) {
         ...
         * Change here.  *
         *****************/
        ArrayList<DirectedEdge<Node>> path = new ArrayList<>();

        while (entry.backPointer != null){
            path.add(0,entry.lastEdge);
    ...
    private class PQEntry {
        ...
        PQEntry(Node node, double costToHere, DirectedEdge<Node> lastEdge, PQEntry backPointer, double costToGoal) {
            this.node = node;
